...
while True:
    ...
    if shape[0]>=4:
        ...
        dst1 = cv2.warpPerspective(img5,H,(300,300))
        identity=id_detect(dst1)
        cv2.putText(frame,str(identity),(corners[0][0][0],corners[0][0][1]), font, .6,(0,0,255),4) 
        cv2.imshow('d',dst1)
    ...
    if shape[0]>=4:
        ...
        img5=cv2.drawContours(frame,cnt,-1,-1)
        H=homography(world_x,world_y,camera_x,camera_y)      #
        dst2 = cv2.warpPerspective(img5,H,(300,300))
        identity=id_detect(dst2)
        cv2.putText(frame,str(identity),(corners[0][0][0],corners[0][0][1]), font, .6,(0,0,255),4)         
        cv2.imshow('d33',dst2)    
    shape=np.shape(corners)
    if shape[0]>=4:
        ...
        img5=cv2.drawContours(frame,cnt,-1,-1)
        H=homography(world_x,world_y,camera_x,camera_y)    
        dst3 = cv2.warpPerspective(img5,H,(300,300))
        identity=id_detect(dst3)
        cv2.putText(frame,str(identity),(corners[0][0][0],corners[0][0][1]), font, .6,(0,0,255),4)
        cv2.imshow('dd',dst3)
